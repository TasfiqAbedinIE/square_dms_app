...
class _NonProductiveTimeMainScreenState
  ...
  /// Upload NP entries for the selected date (unchanged)
  ...
  //     final insertPayload = newEntries
  //         .map(
  //           (e) => {
  //             'id': e['id'],
  //             'lineNo': e['lineNo'],
  ...
  /// Helper: fetch distinct values for a given column from Supabase’s Sales_Order_Data
  Future<List<String>> _fetchDistinctColumn(String columnName) async {
    final client = Supabase.instance.client;
    final data = await client
        .from('Sales_Order_Data')
        .select(columnName); // fetch all rows’ values for that column

    // Map to a list of strings, then convert to a Set to remove duplicates:
    final List<String> allValues =
        data.map((e) => e[columnName] as String).toList();

    return allValues.toSet().toList()..sort();
  }
